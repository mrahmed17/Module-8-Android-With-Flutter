...
public class AuthenticationController {
    ...
    public ResponseEntity<AuthenticationResponse> register(
            ...
            @RequestParam(defaultValue = "EMPLOYEE") Role role) {  // Default to "EMPLOYEE"
        try {
            String profilePhotoPath = userService.saveProfileImage(profilePhoto);
            User user = new User(name, email, password, cell, address, gender, LocalDate.parse(dob), 0.0, LocalDate.now(), true, false, profilePhotoPath, role);
